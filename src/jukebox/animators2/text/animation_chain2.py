...
class AnimationChain2(TextAnimatorBase):
...
async def main():
    ...
    links = [
        AnimationChainLink2(MultiLineGenerator, onFinished=on_multiline_finished),
        AnimationChainLink2(Slide, onFinished=on_random_typewriter_finished),
    ]

    anim = AnimationChain2(links=links, text="Hello there! My name is Slim Shady.", max_text_width=12)
    await anim.Start()
